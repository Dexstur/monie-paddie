...
import { InputHead, Label, InputField } from "../signup/Signup.style";
...
function QuickTransfer() {
  ...
  const [bankDetails, setBankDetails] = useState({ name: "", actNo: "" });
  ...
  const [feedback1, setFeedback1] = useState("fetching account details...");
  ...
  function submitForm2(e: FormEvent<HTMLFormElement>) {
    ...
    if (!submit1) {
      ...
      Api.post("/transactions/transfer", transferDetails)
        .then(() => {
  ...
  useEffect(() => {
    let bankName = "";
    Api.get(`/transactions/transfer?id=${id}`)
      .then((res) => {
        const { message } = res.data;
        setData(message);
        const details: TransactionItem = res.data.data;
        setBankDetails({
          name: details.bankName,
          actNo: details.accountNumber || "",
        });
        ...
        axios.get("https://api.paystack.co/bank").then((res) => {
          ...
          setBankList(fresh);
          const prefill = fresh.filter((bank) => bank.label === bankName);
          selectBank(prefill[0]);
          setAcctNum(bankDetails.actNo);
        });
        setStep(1);
        setFeedback1("");
      })
      ...
      <Layout activeNav="payment" history={history}>
        ...
        <div style={{ padding: "40px 0" }}>
          <Wrapper show={step === 1}>
            <Top>
              <LogoWrap>
                <Logo src={WalletImg} alt="load img" />
              </LogoWrap>
